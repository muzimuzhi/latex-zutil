\ProvidesExplFile {zutil-l3extras.code.tex} {2026-08-29} {0.6.0}
...
\cs_new_protected:Npn \zutil_cs_local_undefine:N #1
  {
    \cs_set_eq:NN #1 \tex_undefined:D  % noqa: w200
  }
\cs_new_protected:Npn \zutil_cs_local_undefine:c #1
  {
    \if_cs_exist:w #1 \cs_end:
    \else:
      \use_i:nnnn
    \fi:  % noqa: w302
    \exp_args:Nc \zutil_cs_local_undefine:N {#1}  % noqa: w302
  }
...
\prg_new_conditional:Npnn \zutil_cs_if_defined:N #1 { p, T, F, TF }
  {
    \if_cs_exist:N #1
      \prg_return_true:
    \else:
...
\prg_new_conditional:Npnn \zutil_cs_if_defined:c #1 { p, T, F, TF }
  {
    \if_cs_exist:w #1 \cs_end:
      \prg_return_true:
    \else:
...
% safe c-type expansion which never defines the #2 csname to \relax,
% but needs TeX's execution processor, thus is unexpandable.
%
% a trick learned from https://github.com/latex3/latex2e/commit/ef35caf4324ae6522204917b2da0742ef6a6281f
\cs_new_protected:Npn \zutil_exp_args_safe:Nc #1#2
  {
    \group_begin:
      \exp_after:wN
    \group_end:
...
\cs_new_protected:Npn \__zutil_cs_generate_variant:nnN #1#2#3
  {
    ...
    \bool_if:NT #3
      {
        \tl_set:Ne \l__zutil_tmpa_tl
          { \__zutil_cs_sig_base_form:w #2 \q_stop }
        % skip if it's already the base form
        \str_if_eq:onF { \l__zutil_tmpa_tl } { #2 }
          {
            \zutil_exp_args_safe:Nc \cs_generate_variant:Nn
              { #1: \l__zutil_tmpa_tl } { #2 }
          }
      }
  }
...
% Since LaTeX 2026-06-01, in LaTeX `\c_novalue_tl` is redefined to hold
% `\NoValue`. And `\NoValue` one-step expands to tokens `-NoValue-` in document
% category code régime (both the dashes are in catcode 12, character).
\tl_const:Ne \c_zutil_novalue_tl  % noqa: w415
  { \char_generate:nn { `- } { 11 } NoValue \token_to_str:N - }
