\ProvidesExplFile {zutil-debug.code.tex} {2025-08-24} {0.3.2}
...
\cs_new_protected:Npn \zutil_debug_generate_variant:Nn #1#2
  {
    ...
    \clist_map_inline:nn { #2 }
      {
        \__zutil_debug_gput_collector:ce
          { \zutil_cs_function_name:N #1 : ##1 }
          % the collector of base form #1
...
\cs_generate_variant:Nn \__zutil_debug_gput_collector:Nn { ce }

% all args are curried
\cs_new:Npn \__zutil_debug_collect_args:Nw
  {
    \prop_item:Nn \g__zutil_debug_collectors_prop  % noqa: w429
  }
    ...
    \bool_if:NT \l__zutil_debug_do_bool
      {
        ...
        % \q_no_value
        \seq_pop_left:NN \l__zutil_debug_labels_seq \l__zutil_tmpa_tl
        \zutil_debug_do:e
          {
            % pretend it's an error, so can be filtered out by TeXstudio
            !~debug~
            \tl_if_eq:NNF \l__zutil_tmpa_tl \q_no_value
              {
                % output forms
                % [label1]
