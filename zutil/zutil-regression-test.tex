...
\cs_new_protected:Npn \END
  {
    ...
    \hook_use:n { test/END }
    \test_check_group_level:n { \c_zero_int }
    \test_check_if_level:n { \c_one_int }
    \TYPE { ^^JEND-TEST-LOG^^J }
    \test_end:
  }
\cs_new_protected:Npn \test_check_group_level:n #1
  {
...
\cs_new_protected:Npn \test_check_if_level:n #1
  {
...
\hook_gput_code:nnn { test/TEST/begin } { . }
  {
    \int_gset_eq:NN \g_test_group_level_int \currentgrouplevel
    % enable checking commands
    \cs_set_eq:NN \PASSED \__test_PASSED:
  }
\cs_new:Npn \test_type:e #1
  {
    \iow_term:e { [test]~ #1 }
  }
...
\hook_gput_code:nnn { test/TEST/end } { . }
  {
    ...
    \test_check_group_level:n { \g_test_group_level_int }
    % log error counts
    \test_add_expandable_errors_cnt:
    \int_compare:nNnF \g_test_errors_cnt_int = \g_test_errors_tmp_int
      {
        \test_log_error_counts:n
          { \g_test_errors_cnt_int - \g_test_errors_tmp_int }
      }
    % log check counts
    \flag_if_raised:NT \l_test_checks_cnt_flag
      {
        \int_gadd:Nn \g_test_check_cnt_int
          { \flag_height:N \l_test_checks_cnt_flag }
        \test_log_check_counts:n { \flag_height:N \l_test_checks_cnt_flag }
        \flag_clear:N \l_test_checks_cnt_flag
      }
  }
...
\cs_new_protected:Npn \test_add_expandable_errors_cnt:
  {
    \int_gadd:Nn \g_test_errors_cnt_int
      { \flag_height:N \l_test_exp_errors_cnt_flag }
  }

\cs_new_protected:Npn \test_log_error_counts:n #1
  {
    \test_type:e { \int_eval:n {#1} ~error(s) }
  }
\cs_new_protected:Npn \test_log_check_counts:n #1
  {
    \test_type:e { \int_eval:n {#1} ~check(s) }
  }
...
\cs_new:Npn \__test_PASSED:
  {
    \int_compare:nNnTF { \currentgrouplevel } > { \g_test_group_level_int }
      { \msg_expandable_error:nn { test } { PASSED } }
      { \flag_raise:N \l_test_checks_cnt_flag }
  }
